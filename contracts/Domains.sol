...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Wavy Resolver", "WRV") {
        tld = _tld;
    ...
    function register(string calldata name) public payable {
        require(domains[name] == address(0), "domain taken");
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Wavy Resolver", "image": "data:image/svg+xml;base64,',
    ...
    function setRecord(string calldata name, string calldata record) public {
        require(
            domains[name] == msg.sender,
            "You must own in order to set the record"
        );
        records[name] = record;
    }
